...
(define the-grammar
  '((program
    ...
    (boolexp
     (binary_bop "(" expression "," expression ")")
     bin-bop-exp)

    (boolexp
     (unary_bop "(" expression ")")
     un-bop-exp)
...
(define (value-of-program pgm)
  (cases program pgm
    ...
    [bexp-prg (bexp)
              (value-of-boolexp bexp (init-env))]))

(define (value-of-boolexp bexp env)
  (cases boolexp bexp
    [bin-bop-exp (binop e1 e2)
                 (let   ([v1 (lower (value-of e1 env))]
                         [v2 (lower (value-of e2 env))])
                   (cond  [(string=? binop "equal?") (lift (= v1 v2))]
                          [(string=? binop "greater?") (lift (> v1 v2))]
                          [(string=? binop "less?") (lift (< v1 v2))]
                          [else (eopl:error 'boolexp-binop "binop ~s doesn't exist" binop)]))]

    [un-bop-exp (unop e1)
                (let ([v1 (lower (value-of e1 env))])
                   (cond [(string=? unop "zero?") (bool-val (zero? v1))]
                         [(string=? unop "null?") (bool-val (null? v1))]
                         [else (eopl:error 'un-bop-exp "unop ~s doesn't exist" unop)]))]))
  ...
  (cases expression exp
    ...
    [binaryop-exp (binop e1 e2)
                  (let ([v1 (lower (value-of e1 env))]
                    ...
                    (cond [(string=? binop "-") (lift (- v1 v2))]
                          ...
                          [(string=? binop "cons") (lift (cons (lift v1) v2))]
                          [else (eopl:error 'binary-op-handling "binop ~s doesn't exist" binop)])
                    )]

    [unaryop-exp (unop e1)
                 (let ([v1 (lower (value-of e1 env))])
                   (cond [(string=? unop "minus") (lift (* -1 v1))]
    ...
    [naryop-exp (naryop elst)
                (let ([evlst (map (lambda (e) (value-of e env)) elst)])
                  (cond [(string=? naryop "list") (lift evlst)]
                        [else (eopl:error 'nary-op-handling "naryop ~s doesn't exist" naryop) ]))]
    

    ;;if-exp
    [if-exp (bexp e2 e3)
            (let ([v1 (value-of-boolexp bexp env)])
              (if (lower v1)
                  (value-of e2 env)
                  (value-of e3 env)))]

    [cond-exp (blexplst rtexplst)
              (letrec ([evalb (lambda (e) (lower (value-of-boolexp e env)))]
                       [eval (lambda (e) (lower (value-of e env)))]
                       [loop (lambda (qlst alst) (cond [(null? qlst) (eopl:error 'cond-exp "no case succeeded")]
                                                       [(evalb (car qlst)) (lift (eval (car alst)))]
