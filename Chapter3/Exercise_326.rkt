  ...
  (cases expression exp
    ...
    [proc-exp (varlst body)
              (proc-val ;; converts it to AST
               (procedure varlst body (filter-env env varlst body)))]
...
(define (filter-env env bound-vars pbody)
  (define (collst fenv bvs elst)
    (cond [(null? elst) fenv]
          [else (collst (collect fenv bvs (car elst))
                        bvs
                        (cdr elst))]))
  
  (define (collect fenv bvs exp)
    (cases expression exp
      [const-exp (num)
                 fenv]
      [var-exp (var)
               (if (memv var bvs)
                   fenv
                   (extend-env var (apply-env env var) fenv))]
      [if-exp (e1 e2 e3)
              (collect (collect (collect fenv bvs e1) bvs e2) bvs e3)]

      [let-exp (v e1 body)
               (collect (collect fenv bvs e1)
                        (cons v bvs)
                        body)]

      [proc-exp (varlst body)
                (collect fenv (append varlst bvs) body)]

      [call-exp (rator randlst)
                (collect (collst fenv bvs randlst) bvs rator)]

      [else (eopl:error 'filter-env "Unexpected expression ~s" exp) ]))

  (collect (empty-env) bound-vars pbody))
...
(define (apply-procedure prc arglst)
  (cases proc prc
    ...
    (built-in (name)
              (cond [(eqv? name '-) (letrec ([args (map lower arglst)]
                                             [reduce (lambda (bv lst)
                                                       (cond [(null? lst) bv]
                                                             [else (reduce (- bv (car lst)) (cdr lst))]))])
                                      (lift (reduce (car args) (cdr args))))]
                    [(eqv? name 'zero?) (let* ([args (map lower arglst)]
                                               [check (= (car args) 0)])
                                          (if check
                                              (lift #t)
                                              (lift #f))) ]
                    [(eqv? name '*) (letrec ([args (map lower arglst)]
                                             [fold (lambda (bv lst)
                                                     (cond [(null? lst) bv]
                                                           [else (*  (car lst) (fold bv (cdr lst)))]))])
                                      (lift (fold 1 args)))]
                    [else  (eopl:error 'apply-procedure "Unknown built-in ~s" name) ]))))
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;;              TEST TOOLS     
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
...
let fix =  proc (f)
             (proc(x) (x x)
                proc(x) (f proc(y) ((x x) y)))
in let
    factp = proc (f) proc(x) if (zero? x) then 1 else (* x (f (- x 1)))
in let fact = (fix factp)
   in (fact 5)") 120)
